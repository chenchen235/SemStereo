...
class Compose(object):
    ...
    def __call__(self, input, target1, target2):
        for t in self.co_transforms:
            input,target1,target2 = t(input,target1,target2)
...
#     def __call__(self, inputs,target,target2):
#         h, w, _ = inputs[0].shape
#         th, tw = self.size
#         if w < tw: tw=w
#         if h < th: th=h
...
#         return inputs, target[y1: y1 + th,x1: x1 + tw], target2[y1: y1 + th,x1: x1 + tw]
...
class RandomVdisp(object):
    ...
    def __call__(self, inputs, target1, target2):
